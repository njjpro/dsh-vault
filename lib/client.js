...
// 一个列表，每条是一个凭据条目（Cloudflare / 服务器 / 之后可加 GitHub 等）。
// 非机密字段（如服务器 IP）写 settings 命名空间 dsh-vault；
// 机密字段（API token、密码）写 DSH 凭据库（ref 如 VAULT_CLOUDFLARE_TOKEN）。
//
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // server: 非 secret 的 ip / user / port / publicHost + secret 的 password。
    ...
    const KINDS = {
      ...
      server: {
        ...
        fields: [
          ...
          { key: "publicHost", label: "公网主机 (Cloudflare)", secret: false },
          { key: "sshAlias", label: "SSH 别名", secret: false },
          { key: "cloudflaredPath", label: "cloudflared 路径", secret: false },
      ...
      aliyun: {
        ...
        fields: [
          ...
          { key: "secretKey", label: "Access Key Secret", secret: true },
        ],
      },
      web: {
        label: "网站",
    ...
    const CSS = [
      ...
      ".dv-badge.warn{background:#fdf3e0;color:#a05a00}",
    ...
    function EntryCard(props) {
      ...
      useEffect(() => {
        ...
        if (!api || fieldKeys.length === 0) {
          if (!api) failureRef.current = "remote.credentials \u670d\u52a1\u4e0d\u53ef\u7528";
          setQueryFailed(Boolean(!api));
        ...
        api
          ...
          .then((resp) => {
            if (cancelled) return;
            // DSH >= 2026-09\uff1aremote.credentials.describe(refs) \u2192 { ok, value: { [ref]: {configured,...} } }
            if (!resp || resp.ok !== true) {
              failureRef.current = resp && resp.error ? String(resp.error.message ?? "describe failed") : "describe failed";
              setQueryFailed(true);
              setConfigured(undefined);
              return;
            }
            const view = resp.value ?? {};
            const next = {};
      ...
      const commitSecret = async (fieldKey) => {
        ...
        if (!api) return;
        try {
          const resp = await api.set(secretRef(entry.id, fieldKey), value);
          if (!resp || resp.ok !== true) throw new Error(resp?.error?.message ?? "set failed");
            ...
            React.createElement(
              ...
              isSecret
                ? React.createElement(
                    "span",
                    {
                      className: `dv-badge ${queryFailed ? "warn" : isConfigured ? "on" : "off"}`,
                      title: queryFailed ? (failureRef.current || "向后端查询密钥状态失败，请刷新页面重试") : undefined,
                    },
                    queryFailed ? "查询失败" : isConfigured ? "已配置" : "未配置"
                  )
                : null
            ),
            React.createElement("input", {
              ...
              ? React.createElement(
                  "button",
                  {
                    ...
                    onClick: async () => {
                      ...
                      if (!api) return;
                      try {
                        const resp = await api.unset(secretRef(entry.id, field.key));
                        if (!resp || resp.ok !== true) throw new Error("unset failed");
    ...
    function VaultSection(props) {
      ...
      return React.createElement(
        ...
        React.createElement(
          "p",
          { className: "dv-hint" },
          "说明：改动在失焦（点输入框外）后保存。Cloudflare 填 Token；服务器填 IP、用户、端口、公网主机和密码；阿里云填 Access Key ID 和 Access Key Secret。"
        )
      );
    }
    ...
    function apply(ctx) {
      ...
      const getApi = () => {
        try {
          const remotes = ctx.remote && ctx.remote.credentials ? ctx.remote.credentials : undefined;
    ...
    exports.apply = apply;
    exports.inject = ["settingsScope", "slots", "remote", "remote.credentials"];
