    ...
    function EntryCard(props) {
      ...
      useEffect(() => {
        ...
        api.credentials
          ...
          .then((resp) => {
            if (cancelled) return;
            // 新版 DSH 的 credentials/describe 直接返回 { [ref]: {configured,...} } 平铺 map；
            // 兼容更早版本的 { credentials: {...} } 包装。
            const raw = resp?.result?.ok ? resp.result.value ?? {} : null;
            if (raw === null) {
              setQueryFailed(true);
              setConfigured(undefined);
              return;
            }
            const view = raw.credentials ?? raw;
            const next = {};
              ...
              isSecret
                ? React.createElement(
                    "span",
                    {
                      ...
                      title: queryFailed ? "向后端查询密钥状态失败，请刷新页面重试" : undefined,
                    },
